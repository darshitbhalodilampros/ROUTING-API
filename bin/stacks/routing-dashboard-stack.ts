...
import { ID_TO_NETWORK_NAME } from 'lampros-sor/build/main/util/chains'
...
export class RoutingDashboardStack extends cdk.NestedStack {
  constructor(scope: Construct, name: string, props: RoutingDashboardProps) {
    ...
    const WORKS_ON_CHAIN = [ChainId.MODE]

    const MAINNETS = SUPPORTED_CHAINS.filter((chain) => WORKS_ON_CHAIN.includes(chain))
    ...
    const perChainWidgetsForRoutingDashboard: any[] = _.flatMap([MAINNETS, WORKS_ON_CHAIN], (chains) => [
      {
        height: 8,
        width: 24,
        type: 'metric',
    ...
    const rpcProvidersWidgetsForRoutingDashboard = new RpcProvidersWidgetsFactory(
      NAMESPACE,
      region,
      MAINNETS.concat(WORKS_ON_CHAIN)
    ).generateWidgets()
